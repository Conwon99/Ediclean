...
export const ExteriorWhyChooseSection = ({ serviceSlug, location, ctaHref = "/contact/" }: Props) => {
  ...
  const isRenderSoftwashing = serviceSlug === "render-softwashing";
  const ctaLabel = isRenderSoftwashing ? "GET A FREE QUOTE" : "BOOK NOW";
  const locationLine =
        ...
        <div className="box-border caret-transparent gap-x-7 flex flex-col justify-between gap-y-7 md:gap-x-10 md:flex-row md:gap-y-10">
          ...
          <div className="box-border caret-transparent blur-0 basis-[0%] grow max-w-none md:max-w-[544px]">
            ...
            <div className="text-neutral-700 box-border caret-transparent space-y-4">
              {whyChoose.points.map((point, i) => (
                <div key={i} className="box-border caret-transparent gap-x-3 flex items-start gap-y-2">
                  <div className={`items-center box-border caret-transparent flex h-6 justify-center w-6 overflow-hidden rounded-[50%] shrink-0 mt-0.5 ${isRenderSoftwashing ? "text-white bg-cta-light" : "text-black bg-neutral-200"}`}>
            ...
            <div className="box-border caret-transparent mt-7 md:mt-10">
              ...
              >
                <span className={`relative text-sm z-[2] md:text-base md:leading-6 ${isRenderSoftwashing ? "font-bold" : "font-medium"}`}>{ctaLabel}</span>
